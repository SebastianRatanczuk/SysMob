...
def runDecrease():
    ...
    for image in images_to_decrease:
        ...
        if len(OG.shape) < 3:
            ...
            mean_resize = np.zeros((new_height_decrease, new_width_decrease, 3), dtype=np.uint8)
            mean_weighted_resize = np.zeros((new_height_decrease, new_width_decrease, 3), dtype=np.uint8)
            median_resize = np.zeros((new_height_decrease, new_width_decrease, 3), dtype=np.uint8)
